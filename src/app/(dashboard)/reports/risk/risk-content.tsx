...
import { fetchAiRisk } from "@/hooks/use-ai-api";
...
import { TrendingUp, TrendingDown, Zap, Trophy, Shield } from "lucide-react";
...
export function RiskContent({ trades }: Props) {
  ...
  const stats = useMemo(() => buildRiskStats(trades, useDollar), [trades, useDollar]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-foreground">{t("risk.title")}</h1>
      ...
      {!empty && (
        <AiAgentCard
          title={t("risk.aiCardTitle")}
          description={t("risk.aiCardDesc")}
          icon={<Shield className="h-5 w-5" />}
          onGenerate={() =>
            fetchAiRisk({ importId, accountId, period, locale })
          }
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-xs font-medium">{t("risk.bestRange")}</span>
              </div>
              <p className="mt-1 font-semibold text-profit">{kpis.best.label}</p>
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-xs font-medium">{t("risk.worstRange")}</span>
              </div>
              <p className="mt-1 font-semibold text-loss">{kpis.worst.label}</p>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-xs font-medium">{t("risk.mostActive")}</span>
              </div>
              <p className="mt-1 font-semibold text-foreground">
                {kpis.mostActive.label}
              </p>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-xs font-medium">{t("risk.bestWinRate")}</span>
              </div>
              <p className="mt-1 font-semibold text-foreground">
                {kpis.bestWr.label}
              </p>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-1.5 text-base font-semibold">
                Net P&L e Trade Count por faixa R
                <WidgetTooltip text="Lucro e prejuízo líquido por faixa de R-múltiplo." />
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-1.5 text-base font-semibold">
                Win % por faixa R
                <WidgetTooltip text="Taxa de vitória por faixa de R-múltiplo." />
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="h-64 w-full sm:h-72">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="pb-2 text-left font-medium text-muted-foreground">Faixa R</th>
                  ...
                  {stats.map((s) => (
                    <tr key={s.label} className="border-b border-border/50">
                      ...
                      <td className="py-2 text-right text-loss">
                        {s.avgLoss > 0 ? fmtPnl(-s.avgLoss, useDollar) : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {!empty && stats.length === 0 && (
        <Card>
          <CardContent className="flex min-h-[120px] flex-col items-center justify-center py-8">
            <p className="text-sm text-muted-foreground">
              Nenhum trade com R-múltiplo registrado para exibir por faixa.
            </p>
          </CardContent>
        </Card>
      )}
