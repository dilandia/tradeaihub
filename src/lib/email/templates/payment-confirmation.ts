...
export function paymentConfirmationEmailHtml(params: PaymentConfirmationParams): string {
  ...
  const t = {
    preheader: isPt
      ? `Pagamento confirmado — Plano ${params.planName} ativado`
      : `Payment confirmed — ${params.planName} Plan activated`,
    heading: isPt
    ...
    labelPlan: isPt ? `Plano` : `Plan`,
    labelAmount: isPt ? `Valor` : `Amount`,
    labelNext: isPt ? `Proxima cobranca` : `Next billing`,
    cta: isPt
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#1e293b;border-radius:8px;overflow:hidden;">
                <tr>
                  <td style="padding:20px 24px;border-bottom:1px solid #334155;">
                    <p style="margin:0;font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#64748b;font-weight:600;">${t.labelPlan}</p>
                    <p style="margin:4px 0 0;font-size:18px;font-weight:700;color:#a855f7;">${params.planName}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:16px 24px;border-bottom:1px solid #334155;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td>
                          <p style="margin:0;font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#64748b;font-weight:600;">${t.labelAmount}</p>
                          <p style="margin:4px 0 0;font-size:16px;font-weight:600;color:#22c55e;">${params.amountPaid}</p>
                        </td>
                        <td align="right">
                          <p style="margin:0;font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#64748b;font-weight:600;">${t.labelNext}</p>
                          <p style="margin:4px 0 0;font-size:16px;font-weight:600;color:#e2e8f0;">${params.nextBillingDate}</p>
                        </td>
                      </tr>
                    </table>
                  </td>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto;">
                <tr>
                  <td style="border-radius:8px;background:linear-gradient(to right,#6366f1,#7c3aed);">
                    <a href="${settingsUrl}" target="_blank" style="display:inline-block;padding:14px 32px;font-size:16px;font-weight:600;color:#ffffff;text-decoration:none;border-radius:8px;">${t.cta}</a>
                  </td>
                </tr>
              </table>
            </td>
